...
export class ApprovalStagesListService {
  ...
  async update(
    ...
    const saved = await this.approvalStagesListRepository.save(approval);
    ...
    if (createAuditTrail) {
      await this.auditTrailService.create(
        {
          service: "ApprovalStagesListService",

          method: "update",

          raw_data: JSON.stringify(updateDto),

          description: `Updated approval stage id ${id}`,
    ...
    try {
      ...
      this.sseEventEmitter.emitUpdateSignal("approval-stageslist", saved.id);
    } catch (err) {
  ...
  async toggleStatus(
    ...
    if (!approval) {
      throw new NotFoundException(`Approval stage with ID ${id} not found`);
    }
    ...
    await this.auditTrailService.create(
      {
        ...
        description: `Toggled approval stage status to ${newStatusName}`,
    ...
    try {
      ...
      this.sseEventEmitter.emitUpdateSignal("approval-stageslist", id);
    } catch (err) {
    ...
    await this.auditTrailService.create(
      {
        ...
        description: `Bulk updated approval stages`,
